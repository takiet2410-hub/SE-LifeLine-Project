...
### 2.5 `ScreeningFormTemplate` (collection: `screening_form_templates`)
Stores the dynamic questionnaire blueprint.

| Field | Type | Notes |
| :--- | :--- | :--- |
| `versionName` | string | Unique identifier for this version |
| `isActive` | boolean | Only one template active at a time |
| `questions` | array of Question | `questionId`, `questionText`, `isMultiSelect`, `options` |
| `options.outcomeFlag` | enum(`PASS`,`REVIEW`,`REJECT`) | Configures dynamic eligibility |

### 2.6 `ScreeningForm` (collection: `screening_forms`)
Traced to SYS-UC-01, LL-UC-07 Steps 5–6.

| Field | Type | Notes |
| :--- | :--- | :--- |
| `appointmentId` | ObjectId FK → Appointment | 1:1 |
| `responses` | array of QuestionAnswer | Answers to questionnaire (questionId, selectedOptions, description) |
| `outcome` | enum(`PASS`,`REVIEW`,`REJECT`) | Auto-computed based on Vietnamese blood donation eligibility guidelines |

### 2.7 `ETicket` (collection: `e_tickets`)
