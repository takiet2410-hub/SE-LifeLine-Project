    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden border border-slate-200">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/50">
          <h3 className="font-semibold text-slate-900">Edit Blood Bag Status</h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          {isTerminal ? (
            <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-start gap-2.5 text-amber-800 text-sm">
              ...
              <span>
                Blood bag is currently <strong>{currentStatus}</strong> (terminal state). Status cannot be changed.
              </span>
            </div>
          ) : (
            <>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="e.g. Reserved for emergency surgery at Cho Ray Hospital"
                  rows={3}
