      ...
      if (testResult === 'Pass') {
        toast.success(
          shouldUpdateBloodType
            ? `🎉 Cập nhật nhóm máu (${editBloodType}) & Hoàn tất xét nghiệm: Pass! Túi máu đã được tự động nhập kho.`
            : `🎉 Hoàn tất xét nghiệm: Pass! Túi máu đã được tự động nhập kho (Stock In).`
        );
      } else if (testResult === 'Rejected') {
        toast.info(`🔴 Đã ghi nhận kết quả: Rejected. Mẫu máu không đạt tiêu chuẩn (không nhập kho).`);
      } else {
        toast.success(
          shouldUpdateBloodType
