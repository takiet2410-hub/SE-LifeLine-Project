    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-brand-bg-card p-5 rounded-2xl border border-brand-border shadow-sm flex items-start gap-4">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-brand-text-muted">Tổng yêu cầu</p>
        ...
        <div className="bg-brand-bg-card p-5 rounded-2xl border border-brand-border shadow-sm flex items-start gap-4">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-brand-text-muted">Mức độ nguy kịch</p>
        ...
        <div className="bg-brand-bg-card p-5 rounded-2xl border border-brand-border shadow-sm flex items-start gap-4">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-brand-text-muted">Chờ xử lý</p>
        ...
        <div className="bg-brand-bg-card p-5 rounded-2xl border border-brand-border shadow-sm flex items-start gap-4">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-brand-text-muted">Đã đáp ứng</p>
      ...
      <div className="bg-brand-bg-card rounded-2xl border border-brand-border shadow-sm overflow-hidden">
        <div className="p-5 border-b border-brand-border flex flex-col sm:flex-row justify-between gap-4">
          <h2 className="text-lg font-bold text-brand-text-main">Danh Sách Yêu Cầu SOS Gần Đây</h2>
          <div className="flex gap-3">
            <div className="relative flex-1 sm:flex-initial">
              <Search className="w-4 h-4 text-brand-text-muted absolute left-3.5 top-1/2 -translate-y-1/2" />
              <input 
                type="text" 
                placeholder="Tìm theo mã yêu cầu hoặc bệnh nhân..." 
                value={searchTerm}
          ...
          <div className="border-b border-brand-border bg-brand-bg-muted/40 px-5 py-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[minmax(180px,240px)_minmax(180px,240px)_auto] gap-4 items-end">
              <label className="block">
                <span className="mb-1.5 block text-xs font-semibold text-brand-text-secondary">Nhóm máu</span>
                <select
                  aria-label="Lọc theo nhóm máu"
              ...
              <label className="block">
                <span className="mb-1.5 block text-xs font-semibold text-brand-text-secondary">Mức độ khẩn cấp</span>
                <select
              ...
              <button
                ...
                className="h-10 rounded-lg border border-brand-border-dark bg-white px-4 text-sm font-semibold text-brand-text-secondary hover:bg-brand-bg-muted disabled:cursor-not-allowed disabled:opacity-50 cursor-pointer"
              >
                Xóa bộ lọc
              </button>
            </div>
        ...
        {isLoading && (
          <div className="p-8 text-center">
            ...
            <p className="text-brand-text-muted mt-2">Đang tải danh sách yêu cầu SOS...</p>
          </div>
        )}
                ...
                {requests.length > 0 ? (
                  requests.map((req) => {
                    ...
                    <tr key={reqId} className="hover:bg-brand-bg-muted/30 transition-colors">
                      ...
                      <td className="px-6 py-4 text-brand-text-muted">{req.createdAt ? format(new Date(req.createdAt), 'dd/MM/yyyy HH:mm') : ''}</td>
                      <td className="px-6 py-4 text-right">
                        <button 
                          onClick={() => navigate(isHospital ? `/hospital/sos-requests/${reqId}` : `/bc/sos-requests/${reqId}`)}
                          className="p-2 text-brand-primary hover:bg-brand-primary/10 rounded-lg transition-colors inline-flex cursor-pointer"
                          title="Xem chi tiết"
                        >
                  ...
                  <tr>
                    <td colSpan={7} className="px-6 py-12 text-center text-brand-text-muted">
                      Không tìm thấy yêu cầu SOS nào phù hợp.
                    </td>
                  </tr>
                )}
              </tbody>
