...
export const RegistrationListPage: React.FC = () => {
  ...
  const columns: Column<RegistrationData>[] = [
    ...
    },
    {
      ...
      accessor: (row: RegistrationData) => {
        ...
        if (savedResult === 'Pass' || savedResult === 'Passed' || (row.status === 'Completed' && savedResult !== 'Rejected')) {
          return (
        ...
        if (savedResult === 'Rejected' || row.status === 'Ineligible' || (row.status as string) === 'Ineligible for Donation' || row.status === 'Rejected') {
          return (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white border border-[#f1f3f5] p-4 rounded-2xl shadow-2xs">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-sm font-bold text-[#271816]">
                Chiến dịch: {campaign?.name || 'Tất cả các đợt tiếp nhận máu'}
              </span>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto flex-wrap">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="h-10 px-3 text-[12.5px] font-bold bg-white border border-slate-200 focus:border-[#93000b] rounded-xl text-[#271816] outline-none cursor-pointer shadow-2xs"
            >
